...
class QueryResult {
    public:
        ...
        QueryResult(shared_ptr<StrVec> textOrig, const string schOrig, set<size_t> lineOrig, int countOrig) :
            text(textOrig), search(schOrig), line(lineOrig), count(countOrig) {};

        QueryResult(shared_ptr<StrVec> textOrig, const string schOrig, set<size_t> lineOrig) :
            QueryResult(textOrig, schOrig, lineOrig, 0) {};

        void print() {
            cout << search << " occurs " << line.size() << " times" << endl;
            for (auto &pos : line) {
                cout << "(line " << pos + 1 << ") " << *(text->begin() + pos) << endl;
            }
        };
    private:
...
class TextQuery {
    public:
        TextQuery() : text(make_shared<StrVec>()), clew(make_shared<map<string, set<size_t>>>()) {};
